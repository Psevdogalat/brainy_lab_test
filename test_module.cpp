...
GAME_OBJECT_WALL* object1;
GAME_OBJECT_WALL* object2;
GAME_OBJECT_WALL* object3;
GAME_OBJECT_WALL* object4;
...
void ENGINE::init_scene(){
	...
	*/
	...
	model.push_color(RGB_COLOR(1.0f, 1.0f, 0.0f));
	object1->set_graphic_model(&model);
	object1->set_collision_model(new COLLISION_MODEL(shape1));
	//object1->set_visible(false);
	
	object2 = (GAME_OBJECT_WALL*) spawn(new GAME_OBJECT_WALL(), VECTOR2D(0.0, 0.0), create_normal());
	model.set_vector_object(&shape2);
	model.clear_color_list();
	model.push_color(RGB_COLOR(0.0f, 1.0f, 0.0f));
	object2->set_graphic_model(&model);
	object2->set_collision_model(new COLLISION_MODEL(shape2));
	//object2->set_visible(false);
	
	object3 = (GAME_OBJECT_WALL*) spawn(new GAME_OBJECT_WALL(), VECTOR2D(0.0, 0.0), create_normal());
	model.set_vector_object(nullptr);
	model.clear_color_list();
	...
	object4 = (GAME_OBJECT_WALL*) spawn(new GAME_OBJECT_WALL(), VECTOR2D(0.0, 0.0), create_normal());
	model.set_vector_object(nullptr);
	model.clear_color_list();
...
void ENGINE::compute(double Frame_time){
	...
	if(key_state(KEY_SHIFT)){
		if(key_front_down(KEY_ARROW_LEFT)){
			normal = scene.camera.get_normal();
			normal.rotate(camera_rot_quant);
			scene.camera.set_normal(normal);
		}
	...
	if(key_state(KEY_Q)){
		normal = mov_object->get_normal();
		mov_object->set_normal(normal.rotate(M_PI/32));
	}
